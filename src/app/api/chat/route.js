  ...
  try {
    ...
    const response = await fetch('https://api.groq.com/openai/v1/chat/completions', {
      ...
      headers: {
        ...
        'Authorization': `Bearer ${process.env.GROQ_API_KEY}`, // Use your GROQ API key from the environment variable
      },
      body: JSON.stringify({
        ...
        try {
          ...
          while (true) {
            ...
            while (true) {
              ...
              let jsonString = buffer.substring(0, boundary).replace(/^data: /, '');
              buffer = buffer.substring(boundary + 1);

              // Handle special cases like the '[DONE]' token
